...
def add_reaction(post_id, username, reaction):
    ...
    if existing:
        ...
        new_r = Reaction(post_id=post_id, username=username, reaction=reaction)
...
def get_reactions_summary(post_id):
    results = (
        db.session.query(Reaction.reaction, func.count(Reaction.reaction))
...
def remove_reaction(post_id, username, reaction):
    target = Reaction.query.filter_by(
        post_id=post_id, username=username, reaction=reaction
    ).first()
    if not target:
        return False
