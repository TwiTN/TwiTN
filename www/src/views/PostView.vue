...
const buildThread = (replies, depth = 0, acc = [], parent = null) => {
  replies.forEach((reply) => {
    acc.push({
      ...
      depth,
      parent,
      parentHandle: parent?.author?.username || null,
    });
...
const threadReplies = computed(() =>
  buildThread(post.value?.replies || [], 0, [], post.value),
);
...
const parentThread = computed(() => {
  ...
  const parentHandle = chain.length
    ? chain[chain.length - 1]?.author?.username || null
    : null;
  items.push({
    post: post.value,
    depth: chain.length,
    ...
    <div v-else-if="post" class="space-y-6">
      ...
      <div class="flex items-center gap-3">
        <button
          v-if="currentUser"
          type="button"
          class="btn btn-sm btn-outline border-white/30 text-white/80 hover:bg-white hover:text-black"
          @click="openReply"
        >
          Répondre
        </button>
        <RouterLink
          v-else
          to="/login"
          class="btn btn-sm bg-white text-black hover:bg-white/80"
        >
          Connexion
        </RouterLink>
        <span v-if="!currentUser" class="text-white/60 text-sm"
          >Connecte-toi pour répondre.</span
        >
      </div>
