...
const defaultReactions = baseReactions.map((reaction) => {
  ...
  const display = reaction.emojiPresentation
    ? String.fromCodePoint(reaction.codepoint, 0xfe0f)
    : emoji;
  return { emoji, display, label: reaction.label };
});

const defaultEmojiSet = new Set(
  defaultReactions.map((reaction) => reaction.emoji),
);
...
const react = async (reaction) => {
  ...
  try {
    ...
    const res = await apiFetch(
      `/api/posts/${props.post.id}/reactions/${encoded}`,
      {
        method: "POST",
      },
    );
...
<template>
  <div class="flex flex-wrap items-center gap-2 text-sm text-white/80">
    ...
    <details ref="dropdownRef" class="dropdown dropdown-top">
      <summary
        class="btn btn-ghost btn-sm text-white/70 hover:text-white list-none normal-case"
      >
        <span class="material-symbols-outlined normal-case">add_reaction</span>
      </summary>
      <div
        class="dropdown-content z-20 w-64 rounded-xl border border-white/10 bg-black/80 p-3 shadow-xl backdrop-blur"
      >
        ...
        <div class="flex items-center gap-2">
          <input
            ...
            maxlength="2"
            placeholder="emoji"
            class="input input-xs bg-white/10 text-white placeholder-white/50 border-white/10 w-20"
            @input="handleCustomInput"
            @keyup.enter="submitCustom"
          />
          <button
            class="btn btn-xs bg-white text-black hover:bg-white/80"
            :disabled="loading"
            @click="submitCustom"
          >
            Ajouter
        ...
        <div v-if="customError" class="text-xs text-amber-300 mt-1">
          {{ customError }}
        </div>
      </div>
    </details>
